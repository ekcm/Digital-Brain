...
const flattenColors = (
  ...
  return Object.entries(obj).reduce(
    (acc: { name: string; value: string }[], [key, value]) => {
      if (typeof value === 'string') {
        return [...acc, { name: prefix + key, value }]
      }
      return [
        ...acc,
        ...flattenColors(value as ColorObject, `${prefix}${key}-`),
      ]
    },
    []
  )
...
export default function ColorsPage() {
  ...
  const gradients = [
    ...
    {
      name: 'gradient-peach-lilac',
      value: backgroundImage['gradient-peach-lilac'],
    },
  ]
